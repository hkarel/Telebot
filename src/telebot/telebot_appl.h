...
public slots:
    ...
    void adjacentMessageDel(qint64 chatId, qint64 userId, qint32 messageId);
...
private:
    ...
    struct AdjacentMessage
    {
        ...
        struct Compare
        {
            ...
            int operator() (const tuple<qint64 /*chat id*/, qint64 /*user id*/, qint32 /*message id*/>* item1,
                            const AdjacentMessage* item2) const
            {
                ...
                LIST_COMPARE_MULTI_ITEM(std::get<2>(*item1), item2->messageId)
                return 0;
            }
        };
    ...
    // Сервисная функция для удаления смежных спам-сообщений
    void adjacentMessageDelete(AdjacentMessage*);
